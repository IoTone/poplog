...
\item \textbf{VS Code} and \textbf{Zed} (\code{editors/vscode/},
  \code{editors/zed/}): syntax highlighting, outline and bracket support for
  \code{.p}, \code{.pop11} and \code{.ph}, both built on the
  \code{tree-sitter-pop11} grammar. VS Code installs from the
  \code{pop11-vscode-\emph{v}.vsix} on the releases page; Zed as a dev
  extension pointed at \code{editors/zed}. Either client can run the LSP
  server above for real-compiler diagnostics. An Emacs mode with a live
  listener is in \code{editors/emacs/}.
